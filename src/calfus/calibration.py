...
def model_calibration(S, LVAL, app_prior, act_dcf_raw, min_dcf_raw):
    ...
    for emp_training_prior in emp_training_priors:
        ...
        min_dcf, act_dcf, _ = map(
            Evaluator.evaluate(scores_cal,LPR,LVAL_kf,eff_prior=app_prior)["results"].get,
            ["min_dcf", "dcf", "llr"]
        )
...
def calibration_task(model_results, LVAL, app_prior, bayes_errors_raw, effective_prior_log_odds, log_odd_application):
    ...
    for model in model_results:
        ...
        if SAVE:
            plot_bayes_errors(
                effective_prior_log_odds, [err_min_dcf_raw, err_min_dcf_cal], [err_act_dcf_raw, err_act_dcf_cal],
                log_odd_application, "Comparison between calibrated and uncalibrated model", f"Model: {model}",
                "Prior log-odds", "DCF value", PLOT_PATH_CAL_FUS, f"bayes_error_calibration_{model}", "png", ["Raw", "Cal."]
            )
